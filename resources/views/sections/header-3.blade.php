...
@php
  ...
  $panelLogoTextClass = 'inline-block max-w-[140px] truncate font-heading text-lg font-semibold leading-none text-heading';
@endphp
    ...
    <div class="flex items-center justify-between px-lg h-16 border-b border-border shrink-0">
      <a class="font-semibold text-lg text-heading" href="{{ home_url('/') }}">
        @if ($hasLogo)
          ...
          />
        @else
          <span class="{{ $panelLogoTextClass }}" x-show="!dark">{{ $siteName }}</span>
        @endif
        @if ($hasDarkLogo)
          <img
            class="!h-7 w-auto max-w-[140px] object-contain"
          ...
          />
        @else
          <span class="{{ $panelLogoTextClass }}" x-show="dark" x-cloak>{{ $siteName }}</span>
        @endif
      </a>
      <button
        @click="navOpen = false"
